/**
 * Edge TTS 高清服务端后端（免费、无需密钥、支持 en-US / en-GB / zh-CN）
 *
 * 复刻 edge-tts 官方协议（rany2/edge-tts）：
 *  - WSS: speech.platform.bing.com/consumer/speech/synthesize/readaloud/edge/v1
 *  - Sec-MS-GEC 令牌 = SHA256( Windows-FileTime 5分钟取整 + TRUSTED_CLIENT_TOKEN )
 *  - 发送 TEXT 帧 speech.config + SSML，接收 audio / turn.end
 *
 * 跨运行时（EdgeOne Node / Cloudflare Workers）兼容：不使用 Buffer，
 * 全部使用 Web 标准 API（WebSocket / crypto.subtle / Uint8Array）。
 *
 * 注意：该接口为非官方接口，仅作"高清发音"增强使用；前端已内置
 * Web Speech API 兜底，本函数不可用时自动降级，不影响学习流程。
 */
...
async function secMsGec() {
  ...
  ticks += 11644473600; // Windows epoch
  ticks -= ticks % 300; // round down to 5 minutes
  ticks *= 1e7; // 100-nanosecond intervals
...
async function synthesize(text, voice, rate) {
  ...
  if (!audio && chunks.length === 0) throw new Error('Edge TTS produced no audio');
